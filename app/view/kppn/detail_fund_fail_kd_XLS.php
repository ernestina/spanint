...
//p2
$objPHPExcel->getActiveSheet()->setCellValue('B4', "Tanggal Error");
//p3
$objPHPExcel->getActiveSheet()->setCellValue('C4', "Satker");
//p4
$objPHPExcel->getActiveSheet()->setCellValue('D4', "KPPN");
//p5
$objPHPExcel->getActiveSheet()->setCellValue('E4', "Akun");
//p6
$objPHPExcel->getActiveSheet()->setCellValue('F4', "Program");
//p7
$objPHPExcel->getActiveSheet()->setCellValue('G4', "Output");
//p8
$objPHPExcel->getActiveSheet()->setCellValue('H4', "Dana");
//p9
$objPHPExcel->getActiveSheet()->setCellValue('I4', "Pagu Saat Ini");
//p10
$objPHPExcel->getActiveSheet()->setCellValue('J4', "Pagu Usulan Revisi");
//p11
$objPHPExcel->getActiveSheet()->setCellValue('K4', "Total Kontrak");
//p12
$objPHPExcel->getActiveSheet()->setCellValue('L4', "Blokir");
//p13
$objPHPExcel->getActiveSheet()->setCellValue('M4', "Realisasi");
//p14
$objPHPExcel->getActiveSheet()->setCellValue('N4', "Sisa/kurang");
...
if (count($this->data) == 0) {
	...
	$no++;
	$nil['p1']=$no;
	$nil['p2']=$value->get_error_date();
	$nil['p3']=$value->get_satker_code();
	$nil['p4']=$value->get_kppn_code();
	$nil['p5']=$value->get_account_code();
	$nil['p6']=$value->get_program_code();
	$nil['p7']=$value->get_output_code();
	$nil['p8']=$value->get_dana_code();
	
	if ($value->get_pagu_semula()==0){
		$nil['p9']='0';
	}else{
		$nil['p9']=$value->get_pagu_semula();
	}
	if ($value->get_pagu_revisi()==0){
		$nil['p10']='0';
	}else{
		$nil['p10']=$value->get_pagu_revisi();
	}
	if ($value->get_blokir_kontrak()==0){
		$nil['p11']='0';
	}else{
		$nil['p11']=$value->get_blokir_kontrak();
	}
	if ($value->get_blokir()==0){
		$nil['p12']='0';
	}else{
		$nil['p12']=$value->get_blokir();
	}
	if ($value->get_blokir_realisasi()==0){
		$nil['p13']='0';
	}else{
		$nil['p13']=$value->get_blokir_realisasi();
	}
	$nil['p14']=$value->get_pagu_revisi() - $value->get_blokir_kontrak() - $value->get_blokir() - $value->get_blokir_realisasi();
