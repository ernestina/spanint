...
if (count($this->data) == 0) {
	...
	$nil['p1']=$no;
	$nil['p2']=$value->get_tanggal_gl();
	if ($value->get_trn_kbi()==0){
		$nil['p3']='0';
	}else{
		$nil['p3']=$value->get_trn_kbi();
	}
	if ($value->get_rph_kbi()==0){
		$nil['p4']='0';
	}else{
		$nil['p4']=$value->get_rph_kbi();
	}			
	if ($value->get_trn_non_kbi()==0){
		$nil['p5']='0';
	}else{
		$nil['p5']=$value->get_trn_non_kbi();
	}			
	if ($value->get_rph_non_kbi()==0){
		$nil['p6']='0';
	}else{
		$nil['p6']=$value->get_rph_non_kbi();
	}				
