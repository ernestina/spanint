...
// Header
//p1
$objPHPExcel->getActiveSheet()->setCellValue('A4', "URAIAN");
//p1
$objPHPExcel->getActiveSheet()->setCellValue('B4', "APBN");
//p2
$objPHPExcel->getActiveSheet()->setCellValue('C4', "BUN(REALISASI)");
//p3
$objPHPExcel->getActiveSheet()->setCellValue('D4', "KPPN(REALISASI)");
//p4
$objPHPExcel->getActiveSheet()->setCellValue('E4', "JUMLAH(REALISASI)");
//p5
$objPHPExcel->getActiveSheet()->setCellValue('F4', "PERSENTASE(REALISASI)");
...
if (count($this->data) == 0) {
	...
	$nil['p1']=$value->get_deskripsi();
	if ($value->get_apbn()==0){
		$nil['p2']='0';
	}else{
		$nil['p2']=$value->get_apbn();
	}
	if ($value->get_realisasi_bun()==0){
		$nil['p3']='0';
	}else{
		$nil['p3']=$value->get_realisasi_bun();
	}			
	if ($value->get_realisasi_kppn()==0){
		$nil['p4']='0';
	}else{
		$nil['p4']=$value->get_realisasi_kppn();
	}			
	if ($value->get_jumlah()==0){
		$nil['p5']='0';
	}else{
		$nil['p5']=$value->get_jumlah();
	}				
	if	($value->get_apbn() == 0) { 
		$nil['p6']='0.00%';
	} else { 
		$nil['p6']="(".number_format($value->get_jumlah()/$value->get_apbn()*100,2)."%)";
